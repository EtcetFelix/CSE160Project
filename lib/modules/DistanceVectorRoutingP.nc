...
implementation {
    ...
    command error_t DistanceVectorRouting.start() {
        // init rout table for current node
    ...
    event void DVRTimer.fired() {
        if(call DVRTimer.isOneShot()) {
            call DVRTimer.startPeriodic(30000 + (uint16_t) (call Random.rand16()%3000));
        } else {
            // Decrement TTLs
            decrementTTLs();
    ...
    uint8_t findNextHop(uint8_t dest) {
        ...
        for(i = 1; i < numRoutes; i++) {
            if(routingTable[i].dest == dest) {
                if (routingTable[i].cost != MAX_COST) {
    ...
    bool inputNeighbors() {
        ...
        //dbg(ROUTING_CHANNEL, "Routing Table: Inputting Neighbors for Node %d\n", TOS_NODE_ID);
    ...
    void triggerUpdate() {
        ...
        for(i = 0; i < neighborsListSize; i++) {
            //dbg(ROUTING_CHANNEL, "Sending packet routes to neighbor %d\n", neighbors[i]);
            while(j < numRoutes) {
